...
function printCheaperThan(maxPrice, products) {
  console.log(products.filter((product) => product.price < maxPrice));
}
...
function getCheapestProduct(products) {
  return products.reduce(
    (cheapest, product) =>
      cheapest > product.price ? product.price : cheapest,

    products[0].price
  );
}
getCheapestProduct(shopData);
...
function drawCategories(categories) {
  ...
  buttons.forEach((button) => {
    button.addEventListener("click", function () {
      if (lastRedButton && lastRedButton !== button) {
        lastRedButton.classList.remove("active");
      }
...
function drawProductsByCategories(categories, products) {
  let lastRedButton = null;
  categories.forEach((category) => {
    document.body.insertAdjacentHTML(
      "afterbegin",
    ...
    button.addEventListener("click", function () {
      ...
      if (button.classList.contains("active")) {
        ...
        const filtered = products.filter(
          (product) => product.category === button.innerHTML
        );
        render(filtered);
      }
    });
...
const names = getUniqueCategories(shopData);
drawProductsByCategories(names, shopData);
